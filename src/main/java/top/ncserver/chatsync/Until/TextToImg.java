...
public class TextToImg {
    public static File toImg(String text) throws IOException {
        ...
        for (String line: strings) {
            ...
            result=result*128+(line.length()-result)*64;
            if (minX<result) minX=result;
        }
        int Y= strings.length*192;
        minX=minX+128;
        Font font =new Font("仿宋", Font.BOLD,128);
        ...
        for (int i=0;i< strings.length;i++){
            ...
            for (int j = 0; j < nowLine.length(); j++) {
                if (nowLine.charAt(j)=='§'){
                    ...
                    g.drawString(String.valueOf(nowLine.charAt(dex)), nowX, (i + 1) * 180);
                    if(b.matches(Reg)) {
